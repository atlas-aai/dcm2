...
```{r fit-model, echo = FALSE, eval = TRUE, results = "hide"}
gdina_mod <- GDINA::GDINA(dat = fit_dat,
                          Q = data.frame(q_matrix),
                          model = "logitGDINA",
                          control = list(conv.type = "neg2LL"))
...
```{r estimate-gdina-model-fit, echo = TRUE}
...
Similarly, the SRMSR for the $M_2$ statistic suggests the model fits the data well, since the SRMSR statistic is less than .08 [@hu_bentler_1999].
